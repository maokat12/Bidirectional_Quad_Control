...
from proj1_3.code.alt_se3_control import SE3Control
from proj1_3.code.alt_world_traj import WorldTraj

class Sandbox(object):

    def __init__(self, filename, map_type, k_p, k_d, k_w, k_r):
    ...
    def run_Sandbox(self, filename, map_type, k_p, k_d, k_w, k_r):
        ...
        my_se3_control = SE3Control(quad_params, k_p, k_d + 0.5, k_w, k_r)
        ...
        my_world_traj = WorldTraj(world, start, goal, map_type)
        ...
        if visualize:
            ...
            q = state['q']
            ax = axes[0]
            ax.plot(sim_time, q_des[:, 0], 'r', sim_time, q_des[:, 1], 'g', sim_time, q_des[:, 2], 'b', sim_time,
                    q_des[:, 3], 'k')
            ...
            if collision_pts.size > 0:
                ax.plot(collision_pts[0, [0]], collision_pts[0, [1]], collision_pts[0, [2]], 'rx', markersize=36,
                        markeredgewidth=4)
            ax.legend(handles=[
                Line2D([], [], color='black', linewidth=2, label='Trajectory'),
        ...
        return stopped_at_goal, no_collision, flight_time